import { TestBed, getTestBed } from '@angular/core/testing';
import { HttpClientTestingModule, HttpTestingController } from '@angular/common/http/testing';
import { EntidadeService } from 'app/entities/projetoBaseChronus/entidade/entidade.service';
import { IEntidade, Entidade } from 'app/shared/model/projetoBaseChronus/entidade.model';
import { TipoCadastro } from 'app/shared/model/enumerations/tipo-cadastro.model';
import { TipoUnidade } from 'app/shared/model/enumerations/tipo-unidade.model';
import { TipoLicneca } from 'app/shared/model/enumerations/tipo-licneca.model';

describe('Service Tests', () => {
  describe('Entidade Service', () => {
    let injector: TestBed;
    let service: EntidadeService;
    let httpMock: HttpTestingController;
    let elemDefault: IEntidade;
    let expectedResult: IEntidade | IEntidade[] | boolean | null;

    beforeEach(() => {
      TestBed.configureTestingModule({
        imports: [HttpClientTestingModule],
      });
      expectedResult = null;
      injector = getTestBed();
      service = injector.get(EntidadeService);
      httpMock = injector.get(HttpTestingController);

      elemDefault = new Entidade(
        'ID',
        TipoCadastro.GOVERNO,
        TipoUnidade.PUBLICA,
        'AAAAAAA',
        'AAAAAAA',
        'AAAAAAA',
        'AAAAAAA',
        'AAAAAAA',
        'AAAAAAA',
        'AAAAAAA',
        TipoLicneca.GOV,
        0
      );
    });

    describe('Service methods', () => {
      it('should find an element', () => {
        const returnedFromService = Object.assign({}, elemDefault);

        service.find('123').subscribe(resp => (expectedResult = resp.body));

        const req = httpMock.expectOne({ method: 'GET' });
        req.flush(returnedFromService);
        expect(expectedResult).toMatchObject(elemDefault);
      });

      it('should create a Entidade', () => {
        const returnedFromService = Object.assign(
          {
            id: 'ID',
          },
          elemDefault
        );

        const expected = Object.assign({}, returnedFromService);

        service.create(new Entidade()).subscribe(resp => (expectedResult = resp.body));

        const req = httpMock.expectOne({ method: 'POST' });
        req.flush(returnedFromService);
        expect(expectedResult).toMatchObject(expected);
      });

      it('should update a Entidade', () => {
        const returnedFromService = Object.assign(
          {
            tipoCadastro: 'BBBBBB',
            tipoUnidade: 'BBBBBB',
            razaoSocial: 'BBBBBB',
            nomeFantasia: 'BBBBBB',
            cnpj: 'BBBBBB',
            inscricaoEstadual: 'BBBBBB',
            inscricaoMunicipal: 'BBBBBB',
            telefone: 'BBBBBB',
            email: 'BBBBBB',
            tipoLicneca: 'BBBBBB',
            quantLicencas: 1,
          },
          elemDefault
        );

        const expected = Object.assign({}, returnedFromService);

        service.update(expected).subscribe(resp => (expectedResult = resp.body));

        const req = httpMock.expectOne({ method: 'PUT' });
        req.flush(returnedFromService);
        expect(expectedResult).toMatchObject(expected);
      });

      it('should return a list of Entidade', () => {
        const returnedFromService = Object.assign(
          {
            tipoCadastro: 'BBBBBB',
            tipoUnidade: 'BBBBBB',
            razaoSocial: 'BBBBBB',
            nomeFantasia: 'BBBBBB',
            cnpj: 'BBBBBB',
            inscricaoEstadual: 'BBBBBB',
            inscricaoMunicipal: 'BBBBBB',
            telefone: 'BBBBBB',
            email: 'BBBBBB',
            tipoLicneca: 'BBBBBB',
            quantLicencas: 1,
          },
          elemDefault
        );

        const expected = Object.assign({}, returnedFromService);

        service.query().subscribe(resp => (expectedResult = resp.body));

        const req = httpMock.expectOne({ method: 'GET' });
        req.flush([returnedFromService]);
        httpMock.verify();
        expect(expectedResult).toContainEqual(expected);
      });

      it('should delete a Entidade', () => {
        service.delete('123').subscribe(resp => (expectedResult = resp.ok));

        const req = httpMock.expectOne({ method: 'DELETE' });
        req.flush({ status: 200 });
        expect(expectedResult);
      });
    });

    afterEach(() => {
      httpMock.verify();
    });
  });
});
