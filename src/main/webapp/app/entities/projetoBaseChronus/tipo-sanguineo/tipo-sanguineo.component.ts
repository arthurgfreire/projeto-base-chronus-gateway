...
export class TipoSanguineoComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number): void {
    ...
    this.tipoSanguineoService
      ...
      .subscribe(
        (res: HttpResponse<ITipoSanguineo[]>) => this.onSuccess(res.body, res.headers, pageToLoad),
        () => this.onError()
      );
  }
  ...
  trackId(index: number, item: ITipoSanguineo): string {
    // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
    return item.id!;
  }

  registerChangeInTipoSanguineos(): void {
    this.eventSubscriber = this.eventManager.subscribe('tipoSanguineoListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: ITipoSanguineo[] | null, headers: HttpHeaders, page: number): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    this.router.navigate(['/tipo-sanguineo'], {
      queryParams: {
        page: this.page,
        size: this.itemsPerPage,
